...
const moods = [
  { emoji: '😫', label: 'Тяжело', value: 1, color: 'text-destructive' },
  { emoji: '😕', label: 'Так себе', value: 2, color: 'text-streak' },
  { emoji: '😐', label: 'Норм', value: 3, color: 'text-energy' },
  { emoji: '😊', label: 'Хорошо', value: 4, color: 'text-accent' },
  { emoji: '🔥', label: 'Огонь', value: 5, color: 'text-primary' },
];
  ...
  if (submitted) {
    return (
      ...
      >
        ...
        <p className="text-accent text-sm font-semibold">Чекин записан</p>
    ...
    <div className="glass-card rounded-2xl p-5 border border-energy/10">
          ...
          <motion.button
            ...
            className={`flex flex-col items-center gap-1 p-2 rounded-xl transition-all flex-1 ${
              selectedMood === m.value
                ? 'bg-energy/15 border border-energy/30'
      ...
      <AnimatePresence>
        {selectedMood !== null && selectedEnergy !== null && (
          ...
          >
            <textarea
              placeholder="Что повлияло? (опционально)"
              value={note}
