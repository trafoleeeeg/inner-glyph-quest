          ...
          return (
            <motion.div key={ach.id} variants={item}
              className={`flex items-center gap-3 p-3 rounded-xl transition-all ${
                ach.unlocked ? 'glass-card-hover border border-accent/15' : 'bg-muted/20 border border-transparent'
              }`} whileHover={{ x: 2 }}>
              <motion.span className={`text-xl ${ach.unlocked ? '' : 'grayscale opacity-40'}`}
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  <div className="flex-1 h-1.5 bg-muted/50 rounded-full overflow-hidden">
                    <motion.div
                      ...
                      initial={{ width: 0 }} animate={{ width: `${progress}%` }} transition={{ duration: 0.8, delay: 0.2 }}
                      style={ach.unlocked ? { boxShadow: '0 0 8px hsl(140 70% 50% / 0.4)' } : {}} />
                  </div>
                  <span className="text-[9px] font-mono text-muted-foreground whitespace-nowrap">{ach.progress}/{ach.maxProgress}</span>
                </div>
              </div>
            </motion.div>
          );
