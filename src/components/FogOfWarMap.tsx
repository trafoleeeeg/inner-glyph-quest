...
const HEX_ZONES: Omit<ZoneData, "count">[] = [
  { id: "mood", label: "Состояние", emoji: "📡", color: "hsl(180, 100%, 50%)", glowColor: "hsl(180, 100%, 50%)", maxForClear: 14 },
  { id: "missions", label: "Протоколы", emoji: "⚡", color: "hsl(140, 70%, 50%)", glowColor: "hsl(140, 70%, 50%)", maxForClear: 20 },
  { id: "dreams", label: "Сны", emoji: "🌙", color: "hsl(270, 80%, 65%)", glowColor: "hsl(270, 80%, 65%)", maxForClear: 7 },
  { id: "desires", label: "Желания", emoji: "✨", color: "hsl(45, 100%, 55%)", glowColor: "hsl(45, 100%, 55%)", maxForClear: 5 },
  { id: "health", label: "Тело", emoji: "🧘", color: "hsl(25, 95%, 55%)", glowColor: "hsl(25, 95%, 55%)", maxForClear: 14 },
  { id: "social", label: "Связи", emoji: "👥", color: "hsl(260, 80%, 60%)", glowColor: "hsl(260, 80%, 60%)", maxForClear: 10 },
];

// Hexagon layout positions (center hex + 6 surrounding)
const HEX_POSITIONS = [
  { x: 200, y: 180 }, // center - overall
  { x: 200, y: 60 },  // top
  { x: 320, y: 120 }, // top-right
...
const FogOfWarMap = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      // Health = missions with category 'health'
    ...
    <div className="glass-card rounded-2xl p-5 border border-primary/10">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-sm font-semibold text-foreground flex items-center gap-2">
            <span>🗺️</span> Карта Тумана Войны
          </h3>
          <p className="text-[10px] text-muted-foreground font-mono mt-0.5">
            логируй → рассеивай туман → видь больше
          </p>
        </div>
      ...
      <div className="relative">
        <svg viewBox="0 0 400 370" className="w-full max-w-md mx-auto">
          ...
          {zones.map((_, i) => {
            ...
            return (
              <motion.line
                ...
                x2={pos.x} y2={pos.y}
                stroke={zones[i]?.color || "hsl(180,100%,50%)"}
                strokeOpacity={0.05 + clarity * 0.2}
                strokeWidth={1}
                strokeDasharray="4 4"
          ...
          {/* Center hex - overall clarity */}
          <motion.polygon
            points={hexPoints(HEX_POSITIONS[0].x, HEX_POSITIONS[0].y, 48)}
            fill="url(#grad-mood)"
            ...
            style={{ transformOrigin: `${HEX_POSITIONS[0].x}px ${HEX_POSITIONS[0].y}px` }}
          />
          <text x={HEX_POSITIONS[0].x} y={HEX_POSITIONS[0].y - 8} textAnchor="middle" className="text-2xl" fontSize="22">
            🧠
          </text>
            ...
            return (
              <g key={zone.id} onClick={() => setSelectedZone(isSelected ? null : zone.id)} className="cursor-pointer">
                ...
                <motion.polygon
                  ...
                  strokeOpacity={0.15 + clarity * 0.5}
                  initial={{ opacity: 0, scale: 0 }}
                  animate={{ opacity: 1, scale: 1 }}
                ...
                {fogOpacity > 0.05 && (
                  <motion.polygon
                    points={hexPoints(pos.x, pos.y, hexR - 1)}
                    fill="hsl(220, 20%, 6%)"
                    fillOpacity={fogOpacity * 0.75}
                    filter="url(#fog-blur)"
                    initial={{ fillOpacity: 0.9 }}
                    animate={{ fillOpacity: fogOpacity * 0.75 }}
                    transition={{ duration: 1.5, delay: 0.5 + i * 0.1 }}
                    style={{ pointerEvents: "none" }}
                  />
                )}

                {/* Animated particles for cleared zones */}
                {clarity > 0.5 && (
                  <motion.circle
                    cx={pos.x + 15}
                    cy={pos.y - 10}
                ...
                {/* Emoji */}
                <text x={pos.x} y={pos.y - 5} textAnchor="middle" fontSize="18" style={{ pointerEvents: "none" }}>
                  {zone.emoji}
                </text>
        ...
        >
          ...
          <div className="h-1.5 bg-muted/50 rounded-full overflow-hidden">
            <motion.div
              ...
              initial={{ width: 0 }}
              animate={{ width: `${Math.min(selected.count / selected.maxForClear, 1) * 100}%` }}
              transition={{ duration: 0.8 }}
            />
          </div>
          <p className="text-[10px] text-muted-foreground font-mono mt-1.5">
            {selected.count === 0
              ? "Зона в полном тумане. Начни логировать, чтобы рассеять."
              : Math.min(selected.count / selected.maxForClear, 1) >= 1
              ? "✓ Туман полностью рассеян. Ты видишь эту зону ясно."
              : `Ещё ${selected.maxForClear - selected.count} записей для полной ясности.`}
          </p>
        </motion.div>
      )}
    </div>
