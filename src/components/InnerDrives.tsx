...
import { Plus, X, Swords, Scale } from "lucide-react";
...
const InnerDrives = () => {
  ...
  const addDrive = async (preset: typeof DRIVE_PRESETS[0]) => {
    ...
    if (!error) {
      toast.success(`Драйв "${preset.name}" активирован`);
      fetchDrives();
      setShowAdd(false);
    }
  };
  ...
  const removeDrive = async (id: string) => {
    ...
    toast.success("Драйв деактивирован");
  };

  // Calculate balance score: how evenly distributed are strengths
    ...
    <div className="glass-card rounded-2xl p-5 border border-secondary/10">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-sm font-semibold text-foreground flex items-center gap-2">
            <Swords className="w-4 h-4 text-secondary" /> Мета-Дипломат
          </h3>
          <p className="text-[10px] text-muted-foreground font-mono mt-0.5">
            баланс внутренних драйвов
          </p>
        </div>
        {drives.length >= 2 && (
      ...
      {drives.length === 0 ? (
        <div className="text-center py-6">
          <p className="text-xs text-muted-foreground mb-3">
            Определи свои внутренние драйвы — конфликтующие части, которые тянут тебя в разные стороны
          </p>
          <motion.button
            whileTap={{ scale: 0.95 }}
          ...
          >
            <Plus className="w-3 h-3 inline mr-1" /> Добавить драйвы
          </motion.button>
        </div>
      ) : (
        <>
            ...
            {drives.map((drive, i) => (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-mono" style={{ color: colorMap[drive.color] || colorMap.primary }}>
                      {drive.strength}%
                    </span>
                    <button
                ...
                {drive.description && (
                  <p className="text-[9px] text-muted-foreground/50 font-mono mt-0.5">{drive.description}</p>
                )}
              </motion.div>
            ))}
          </div>
          ...
          {imbalanced && dominantDrive && weakestDrive && (
            ...
            >
              <p className="text-[10px] text-destructive/80 font-mono">
                ⚠ Перекос: «{dominantDrive.name}» ({dominantDrive.strength}%) доминирует над «{weakestDrive.name}» ({weakestDrive.strength}%). 
                Мета-Дипломат рекомендует уделить внимание балансу.
              </p>
            </motion.div>
          )}
          ...
          {balanceScore > 70 && drives.length >= 2 && (
            ...
            >
              <p className="text-[10px] text-accent/80 font-mono">
                ✦ Гармония драйвов: {balanceScore}%. Внутренний Мета-Дипломат работает эффективно.
              </p>
            </motion.div>
          )}
          ...
          {drives.length < 6 && (
            ...
            >
              <Plus className="w-3 h-3 inline mr-1" /> Ещё драйв
            </motion.button>
          )}
        </>
      )}
      ...
      <AnimatePresence>
        {showAdd && (
          ...
          >
            <div className="flex items-center justify-between mb-1">
              <p className="text-xs font-semibold text-foreground">Выбери субличность</p>
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {DRIVE_PRESETS
                .filter(p => !drives.some(d => d.name === p.name))
                .map(preset => (
                  <motion.button
                    key={preset.name}
                    whileTap={{ scale: 0.95 }}
                    onClick={() => addDrive(preset)}
