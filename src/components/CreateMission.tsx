...
const CATEGORIES = [
  ...
  { value: 'desire', label: 'Желание', icon: '✨' },
  ...
  return (
    <div>
      ...
      >
        ...
        <span className="text-sm font-semibold">{isOpen ? 'Отмена' : 'Создать миссию'}</span>
      </motion.button>
        ...
        {isOpen && (
          ...
          >
            <div className="glass-card rounded-2xl p-5 border border-primary/10 space-y-3">
              <input placeholder="Название миссии" value={title} onChange={e => setTitle(e.target.value)}
              ...
              {/* XP */}
              <div>
                <p className="text-[10px] text-muted-foreground font-mono uppercase tracking-wider mb-2">XP награда</p>
                <div className="flex flex-wrap gap-1">
                  {xpOptions.map(xp => (
