...
const GlyphPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchBalance = async () => {
      ...
      const scores = [
        Math.min(avgEnergy * 20, 100),
        Math.min((completions.count || 0) * 5, 100),
        Math.min(avgMood * 20 + moodData.length * 3, 100),
        Math.min((posts.count || 0) * 10, 100),
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-4 py-6 space-y-4">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-[10px] text-muted-foreground font-mono">
            визуализация внутреннего состояния • мета-дипломат
          </p>
        </motion.div>
